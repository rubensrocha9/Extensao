...
export class EmployeeDetailComponent implements OnInit {
  ...
  disabledDepartureDate = (current: Date): boolean => {
    const today = new Date();
    const entryDateValue = this.form.get('entryDate')?.value;

    if (entryDateValue === null) {
      return current.getTime() > today.getTime();
    }

    return current.getTime() < entryDateValue.getTime() || current.getTime() > today.getTime();
}
}
